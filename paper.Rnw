...
%  - check that \gs is used instead of Manual 
% Sweave/Knitr init
...
neuroanatomical segmentation is manual delineation by an expert human rater.
However with the availability of increasingly large MRI datasets, the time and
expertise required for manual segmentation becomes prohibitive
\citep{Mazziotta1995,Mazziotta2001,Mazziotta,Pausova2007}. Further, there is
little agreement between researchers regarding how exactly the hippocampus
should be identified in MRI images \citep{Geuze2004} and this has led to efforts
to create an unified hippocampal segmentation protocol \citep{Jack2011}. 
...
data from 10 subjects. In the second method, \citet{Yushkevich2009} acquired
and labelled hippocampal subfields on high-resolution MRI data from post-mortem
medial temporal lobe samples. Using nonlinear registration guided by manually
derived hippocampus masks and specific landmarks, they demonstrate accurate
...
by both \mb and multi-atlas. The computed segmentations are compared to the
...
To validate the \mb algorithm with respect to other established automated
methods, \mb is applied to large dataset from the ADNI project and the
resulting segmentations are compared to those produced by FreeSurfer, FSL,
MAPER and by expert semi-automated manual segmentations (SNT).

\paragraph{ADNI1 dataset revisited} 
The $1919$ images from the {\em \adnidataset} standardized dataset, described
in Experiment 1, were initially considered.  SNT and FreeSurfer hippocampal
volumes were provided by ADNI, along with FreeSurfer quality control data
(guidelines described in \citep{Hartig2010}). Clinical and demographic data for
the entire \adnidataset dataset are shown in Table
\ref{tab:ADNI1-Dataset-Demographics}.

\paragraph{Experiment details} 
\mb was configured with an atlas library composed of the five Winterburn T1
atlases (described in Experiment 2). The optimal size of template library,
registration method, and label fusion method found in Experiment 1 were used.
The FSL FIRST segmentation method was used  via the \verb+run_first_all+ script
according to the FIRST user guide
\footnote{http://fsl.fmrib.ox.ac.uk/fsl/fslwiki/FIRST/UserGuide}.  All images
from the \adnidataset dataset were segmented by both methods. 
...
passing quality control inspection are included in the analysis.  
...
subjects. Specifically, Pearson correlation is used. We also compute the
correlation in hippocampal volume between the existing, established automated
segmentation methods: FSL, FreeSurfer, and MAPER.  Additionally, we evaluate the
...
@
  ...
  indicate an \mb parameter settings yielding a higher mean DSC score than
...
%
%
<<mb-best-kappa, echo=F, cache=FALSE>>=
ants.mean = subset(all_data_mean, reg_method=="ANTS" & method.mb=="Majority Vote" &      templates.mb == 19 & atlases == 9)
mb.best.kappa = mean(ants.mean$k.mb)
@
...
slight decrease in segmentation performance when the number of templates are
set to an even number.  Our initial concern was that this dip in performance
was a by-product of the \mb algorithm itself.  However, this this pattern is
also found in the results of the multi-atlas segmentations we used in our
...
causing the slight bias observed when using an even number of templates. This
is another area where the voting scheme could be used to improve performance.
...
using ADNI data for validation do at most a single round of
...
hierarchical experimentation protocol that involved \todo{complete this
sentence}.  These methods suggest that some post-processing of the final
segmentations would improve accuracy of the segmentation. While that may be
true, there is little consensus regarding how to achieve this.  
...
are under estimated. By comparison, \mb and MAPER are far more conservative in
volume estimation, suggesting these methods may be better suited for estimating
true-positives. However, in this analysis we have only compared methods by
total hippocampal volume, and so more work is needed to understand the full
