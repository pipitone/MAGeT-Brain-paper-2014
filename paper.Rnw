...
# baseline is multi-atlas majority vote (even though we have MA with other methods)
...
ma = rbind(t_ma,a_ma)
mb = rbind(t_mb,a_mb)
ma$templates =  ma$atlases  # since there is no template library, these are equivalent
mb[mb$method != 'majvote',]$templates = mb[mb$method != 'majvote',]$top_n  # top_n is number of templates used
...
ggplot(all_data, aes(x=templates.y,y=k.y, colour=as.factor(atlases))) + 
  ...
  ylab( "mean Kappa" )
@
  \caption{MAGeT brain mean kappa by atlas and template library size}
  \label{ADNI1_xval}
\end{figure}

\begin{figure}
<<ADN1-Xval-k_diff, cache=TRUE>>=
...
\begin{figure}
<<ADN1-Xval-k-by-disease, cache=TRUE>>=
a_20_tmpls = subset(all_data, 
                      reg_method  == "ANTS" &
                      method.y    == "Majority Vote" &
                      templates.y == 20 &
...
\begin{figure}[h]
<<ADNI-src-volumes-boxplot,echo=F,fig=T, fig.align='center',dependson='ADNI-baseline-volumes-prep',cache=T>>=
totals = vols
...
qplot(SNT,value,data=by_snt_volumes,
    ...
    geom_point() +
     geom_text(aes(
       ...
       x=1800,y=10000, hjust=0,size=2), data=data.frame()) + 
#     geom_text(aes(
...
\begin{figure}
<<FEP_volumes,echo=F,fig=T, fig.align='center',cache=T>>=
...
<<WAval-boxplot,cache=T>>=
# dirty dirty
data = read.csv('data/WAval.csv')
orig = subset(data, version == "orig")
voted = subset(data, version == "voted")
origtotal = orig[,2:6] + orig[,7:11]
votedtotal = voted[,2:6] + voted[,7:11]
pchange = abs((origtotal - votedtotal)/origtotal)
diff = cbind(orig[,1],pchange)

melted = melt(diff, id.vars="orig[, 1]")
names(melted) <- c("subject","variable","value")
ggplot(melted, aes(y=value,x=variable,colour=subject,group=subject)) + 
  geom_point(size=3) + 
  geom_lin() + 
