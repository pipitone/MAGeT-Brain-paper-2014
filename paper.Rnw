...
In this paper we will make a distinction between an atlas and a template image.
The term {\it atlas} refers to two image volumes: an intensity image ({\it atlas
image}) and a corresponding manual segmentation image ({\it atlas labels}). The
term {\it template} refers to the {\it use} an any image and corresponding label
image (whether the labels are manual or computed) as a model in the segmentation
of another image.  The terms {\it atlas library} and {\it template library} mean
a set of such images.  Additionally, we will use the terms {\it target} to refer
to an intensity image for which we would like an segmentation.
...
manual segmentations taken from the ADNI1 project (see below for details). In each
validation round, the training set is used as an atlas library for both \mb and
basic multi-atlas. The remaining images are segmented by both methods, and the
resulting segmentations are compared to the manual segmentations for the images.
...
\subparagraph{Label fusion methods}
...
Reported subject segmentation accuracy is averaged over the ten validation
...
for two different input datasets: the four Winterburn atlas subject BRAVO MR
images (referred to as the {\em WA BRAVO} dataset), and the five
Winterburn atlas subject images resampled to 0.9mm-isotropic voxel resolution
(referred to as the {\em WA Resampled} dataset. Each subject in the
...
\paragraph{Evaluation method}  
The segmentation volumes of each hippocampal subregion are compared to the
expert manual segmentations of the unmodified Winterburn atlases.  As a point of
comparison, we also calculate the subregion volumes from the Winterburn atlas
segmentations after downsampling to 0.9mm-isotropic voxels. 
...
\todo{discuss Bland-Altman?  Or bias analysis}
...
All images from the {\em ADNI1:Complete 1Yr 1.5T} standardized dataset described
...
\paragraph{Experiment details} 
\mb is configured with an atlas library composed of the Winterburn T1 atlases
(see Experiment 2). All images from the ADN11 Complete 1Yr 1.5T dataset are
...
qplot(DX,value,data=melt(totals.complete,measure.vars=c("FS", "FSL", "MAPER","MAGeT","SNT")),
      ...
      xlab("Diagnosis") + 
      ylab(expression(paste("Total hippocampal volume (", mm^3, ")")))
@
...
snt.tests <- ddply(totals.complete, c("DX"), function(df) {
  data.frame( corr = cor(df$SNT, df$MAGeT),
              value = -Inf) 
})
...
ggplot(data=melted, aes(x=SNT, y=volume, colour=method)) + 
      ...
      #              label=paste("r==", signif(corr, digits=3))), 
      #      colour='black',parse=TRUE, data=snt.tests) +
      xlab(expression(paste("Manual total hippocampal volume (", mm^3, ")"))) + 
      ylab(expression(paste("Total hippocampal volume (", mm^3, ")")))
@
  \caption{Comparison of hippocampus volumes obtained by FreeSurfer (FS), FSL, 
  MAPER, \mb (MAGeT) and manual (SNT).}
...
<<ADNI-scr-volumes-table,dependson='ADNI-baseline-volumes-prep',cache=T,results='asis'>>=
melted=melt(totals.complete, measure.vars=c("FS", "FSL", "MAPER","MAGeT"), 
            variable.name = "method", value.name = "volume")

snt.tests <- ddply(totals.complete, c("DX"), function(df) {
  round(data.frame( MAGeT = cor(df$SNT, df$MAGeT),
              MAPER = cor(df$SNT, df$MAGeT),
              FS    = cor(df$SNT, df$FS),
              FSL   = cor(df$SNT, df$FSL)
              ), 2)
})

latex(snt.tests, file="", 
      size="scriptsize",
      caption="", row.names="",
      title="")
@
