...
Nomenclature: SZFEP Dataset
SZFEP dataset demographics are shown in table \ref{tab:SZFEP_Demographics}. 
...
To test the effect of parameter settings on the MAGeT brain algorithm, repeated
random sub-sampling cross-validation (RRSCV) was performed using a subset of the
ADNI1 subjects for validation.  This form of cross-validation (described in
...
consists of repeated trials in which items from the dataset are randomly sampled
and assigned to a training set or validation set. In each trial, performance on
the validation set is measured, and then averaged across all trials. This form
of cross-validation allows

With respect to this We performed RRSCV on each combination of parameters listed above: atlas library
size, template library size, registration method, and label fusion method.  

We performed 10 trials per parameter combination.  In each validation trial, the
training set consisted of the images used as atlases, and the validation set
consisted of the images to be segmented. 



The MAGeT brain algorithm and the basic multi-atlas segmentation procedure were
applied to segment the images in the validation set.  Additionally, in each
trial, the single-atlas segmentation was obtained for each atlas-template.  
