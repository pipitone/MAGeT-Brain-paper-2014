...
paper we focus on methods that use manually segmented atlases as anatomical
priors, as these methods achieve some of the best automated hippocampal
segmentation accuracies to-date. This technique was pioneered by []\citep{} with
single-atlas (or model-based) segmentation.  Volumetric image registration is
used to estimate a fit between the neuroanatomy of an atlas and subject images.
Labelling of the subject image is achieved by applying the resulting
transformation to the atlas labels to bring them into the subject image space
(\{em label propagation}). This method is limited in accuracy by the
introduction of {methodological} errors in registration and partial volume
effects in label resampling, and {conceptual} errors introduced when the anatomy
of the atlas is not representative of the subject.

Multi-atlas segmentation techniques address these limitations by combining
segmentation information from a series of expertly segmentated atlases
\citep{Heckemann2006,Heckemann2011,Collins2010,Lotjonen2012,Aljabar2009,Leung2010,Wolz2010}.
Each atlas image is registered to a subject image, and label propagation is
performed to produce several labellings of the subject image (one from each
atlas). A {\it label fusion} technique, such as voxel-wise voting, is used merge
these labels into a definitive segmentation for the subject.  

{\em Atlas selection} techniques are often used to filter out atlases dissimilar
to a subject image in order to reduce {conceptual} errors \citep{Aljabar2009}.
...
ANIMAL nonlinear registration algorithm, normalised mutual information as a
similarity metric for atlas selection, and majority vote for label fusion
\citep{Collins2010}. 

The Alzheimer's Disease Neuroimaging Initiative (ADNI) is a
commonly used benchmarking dataset of MR images of controls and patients with
...
images \citep{Leung2010}. 

The MAPER whole brain segmentation algorithm \citep{Heckemann2006,Heckemann2011},
using 30 atlases, on all ADNI1 baseline images \citep{Heckemann2011}. 

In \citep{Lotjonen2012}, the authors use a proprietary non-linear registration
method based on intensity differences, and post-processing step taking into
account these intensity differences to 
- TODO: explain "tidy up"
tidy up
fused segmentations. Using images from the ADNI1 baseline dataset, they achieved
a mean DSC of 0.885 with 30 atlases. 

The LEAP algorithm, Wolz et al.  is an elegant modification to the basic
multi-atlas strategy: the atlas library is grown, beginning with a set of
manually labelled atlases and successively incorporating unlabelled subject
images after being labelling using multi-atlas techniques\citep{Wolz2010}.  The
sequence in which subject images are labelled is chosen so that the similarity
...
broken up into sequences of smaller deformations.  With an atlas library of 30
MR images, LEAP was used to segment the ADNI1 baseline dataset, achieving a mean
Dice score of 0.85 with manual segmentations.
...
We will use the term {\it atlas} to refer to two image volumes: an intensity
image ({\it atlas image}) and a corresponding manual segmentation image ({\it
atlas labels}). A {\it subject} image is an intensity image for which we would
like an segmentation.  The term {\it template} will be used to refer more
generically to any image and corresponding labelling, manual or computed, when
it is {\it used as} as a model in the segmentation of another image. In other
publications, the terms atlas and template are used interchangeably, but in this
paper we force the distinction to make it clear when an image has a manual
segmentation.  The terms {\it atlas library} and {\it template library} mean a
set of such images.   
...
will refer to this as {\em basic multi-atlas segmentation}, and specific the
basic schema as: 

    1. an atlas library and set of subject images are given as
       input, and the atlas library is used as a template library in the
       following steps; 
    2. each atlas intensity image is nonlinearly registered to each subject
       intensity image; 
    3. label images from each atlas are propagated via the resulting
       transformations to the subject image space; and 
    4. the resulting labels are fused to produce a single, definitive
       segmentation. 
       
The particular registration and voting method used are left unspecified.

MAGeT brain is best understood as an extension of
the basic multi-atlas segmentation schema. An additional stage is included in
which the template library is constructed by labelling a subset of subject
images, rather than by using the atlas library. The choice of subjects used in
the template library can be made to reflect the neuroanatomy or demographics of
the subject set as a whole (for instance, by sampling equally from cases and
controls). 

Labels from each atlas image are propagated to
