...
\subsubsection{Schizophrenia First Episode Patients}
\label{sec:SZFEP-dataset}
...
The previous experiments explore \mb on an Alzheimer's disease dataset (ADNI).  To validate
that \mb algorithm works effectively with other diseased brain images, 
the performance of \mb is measured on a Schizophrenia dataset.

{\bf Dataset evaluated.} The entire Schizophrenia First Episode Patient (SZFEP) 
dataset (see section \ref{sec:SZFEP-dataset}).

{\bf Atlas and template libary.} The atlas library is composed of the Winterburn 
T1 atlas images, with whole hippocampus segmentations (no subregions). The 
template library is composed of 21 images selected at random from the SZFEP dataset. 

{\bf Registration method.} The \ants registration algorithm is used for image
registration. 

{\bf Label fusion.} The majority-vote method is used for label fusion.

{\bf Evaluation.} The manual segmentation protocol used to segment the Winterburn
atlases is similar to, but different from, the protocol used to segment the SZFEP
dataset. Therefore, rather than use an overlap metric, \mb hippocampal volumes
are compared to the corresponding manual segmentation volumes. 
...
{\bf Dataset evaluated.} \mb segmentations are evaluated on two datasets. The 
WA-BRAVO dataset consists of separately acquired 3T T1 BRAVO images (0.9mm-isotropic voxel)
of four of the five Winterburn atlas subjects. The WA-Resampled dataset consists
of the Winterburn atlas images and segmentations downsampled to 0.9mm-isotropic 
voxel resolution.

{\bf Atlas and template libary.} The atlas library is composed of the Winterburn 
T1 atlas images and hippocampal subregion segmentations. The template library 
consists of 15 3T T1 images (0.9mm-isotropic voxels) of healthy subjects as well
as all of the images from the dataset being evaluated. 

\todo{reference, demographics for health control images}.

{\bf Registration method.} The \ants image registration method is used. 

{\bf Label fusion.} The majority vote label fusion method is used.

{\bf Evaluation.}  Leave-one-out-cross-validation (LOOCV) is performed separately 
for each dataset evaluated (WA-BRAVO, and WA-Resampled) as follows. Each subject
in the dataset is segmented by \mb using an atlas library in which that subject's 
image is excluded. The segmentation volumes of each hippocampal subregion are
compared to the expert manual segmentations of the unmodified Winterburn atlases. 
As a point of comparison, we also calculate the subregion volumes from the Winterburn 
