...
Additionally, by shifting the original manual $0.3mm^3$-isotropic voxel
segmentations by one voxel in the x, y, and z direction and then resampling it
to $0.9mm^3$-isotropic voxels we obtain a simulated manual segmentation having a
...
accuracy is an important consideration that few have touched on previously. In
