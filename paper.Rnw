...
to determine the behaviour of \mb{} when segmenting younger and differently
diseased subjects, and to ensure that we have not also overfit our parameters to
the aging or neurodegenerative brain. Next, we consider all of the images
available in the \adnidataset{} sample, and compare \mb{} segmentations to other
popular segmentation algorithms.  Finally, we conduct a leave-one-out validation
of subfield segmentation using the \citet{Winterburn2013} manually segmented
