...
subfield segmentation in T1-weighted images by using Markov random field priors
learned from 10 manual segmentations.  This work, available as part of the
\freesurfer{} package, is limited as the segmentation omits the tail of the
hippocampus and the protocol has yet to be fully validated.
\citet{Yushkevich2009} manually segment hippocampal subfields on high-resolution
(either $0.2mm^3$ isotropic or $0.2mm \times 0.3mm \times 0.2mm$ resolution
voxels) T2-weighted MR images acquired from five post-mortem medial temporal
lobe samples. Then, using nonlinear registration guided by shape-based models of
the subfield segmentations, and manually derived hippocampus masks of the target
images, the authors demonstrate accurate parcellation of hippocampal subfields
in clinical 3T T1-weighted MRI volumes.  Using \multiatlas with bias correction
techniques, \citet{Yushkevich2010} demonstrate a semi-automated method of
subfield segmentation on in vivo focal T2-weighted MR acquisitions of the
temporal lobe. Manual input is only needed to mark mark divisions between the
head, body and tail of the hippocampus on target images. 
...
segmentation. In this experiment, we aim to evaluate \mb{} hippocampal subfield
...
In this experiment, the Winterburn atlases are resampled to $0.9mm^3$-isotropic
voxel images to simulate standard 3T T1-weighted resolution images. Image
subsampling is performed using trilinear subsampling techniques.  

In each round of LOOCV, one atlas is selected and segmented with \mb{} once
using each possible triplet of atlas images from the remaining four unselected
atlases. We chose to use an atlas library with an odd number of images so as to
ensure an unbiased label fusion when using majority voting (see Discussion). 
...
levels(wa.n.sim$label) <-list(
  "CA1"=1, "CA1"=101, "Subiculum"=2,"Subiculum"=102, 
  "CA4/DG"=4,"CA4/DG"=104, "CA2/CA3"=5,"CA2/CA3"=105, 
...
high-resolution Winterburn atlas subjects' images as atlases. 
...
subregion across all subjects and folds of the validation. Mean (standard
deviation) DSC scores of the subregions are: 
  \Sexpr{mean(CA1)}   ($\pm$\Sexpr{sd(CA1)}) for the CA1 region,  
  \Sexpr{mean(CA23)}  ($\pm$\Sexpr{sd(CA23)}) for the CA2/CA3 regions,  
  \Sexpr{mean(CA4DG)} ($\pm$\Sexpr{sd(CA4DG)}) for the CA4/DG regions,  
  \Sexpr{mean(Sub)}   ($\pm$\Sexpr{sd(Sub)}) for the Subiculum region, and
  \Sexpr{mean(SRLM)}  ($\pm$\Sexpr{sd(SRLM)}) for the moleculare regions.
Bland-Altman plots comparing \mb{} to resampled subregion volumes reveal a
proportional bias towards underestimating volumes in all subregions (Figure
...
levels(wa.n.all.vols$variable) <-list(
  "CA1"="X1", "CA1"="X101", "Subiculum"="X2","Subiculum"="X102", 
  "CA4/DG"="X4","CA4/DG"="X104", "CA2/CA3"="X5","CA2/CA3"="X105", 
...
wa.n.xval.bland.limits = ddply(wa.n.all.vols,c("variable"),function(df) {
...
\hline
%%  
\mb{}                     &9 & \Sexpr{snt.mb.kappa.best} & \cite{Pipitone2014}
...
%%
\mb{}                      &9 & \Sexpr{mb.kappa.best}       &\cite{Pipitone2014} 
...
demonstrates that we can reliably reproduce segmentations for the CA1,
subiculum, and CA4/dentate subregions.  That the CA2/CA3 and molecular
layers are less well reproduced should not be surprising as these are extremely
thin and spatially convoluted regions that originally required high-resolution
MRI for identification and so it is likely that the extents of these regions are
well below the resolution and contrast offered by standard T1-weighted images. 
...
% TODO: say that we have shown the viability of template generation, and as such
% it may also serve as a useful addition to existing segmentation techniques
