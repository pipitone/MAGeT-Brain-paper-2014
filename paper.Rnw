...
d = ddply(na.omit(wa.xval.casted), c('region'), function (df) {
  icc = ICC(df[4:5])
  data.frame(
    icc  = icc$results$ICC[3],
    f    = icc$results$F[3],
    lower= icc$results$"lower bound"[3],
    upper= icc$results$"upper bound"[3],
  )
})
...
#wa.n.vols.mean = ddply(wa.n.all.vols,c('subject','variable'),function (df) {
#  data.frame(
#    mb=mean(df$value),
#    X.3=mean(df$'0.3'),
#    X.9=mean(df$'0.9'))})
...
@
  \caption{Similarity of \mb{} segmentation of subregions and resampled Winterburn atlas labels at $0.9mm^3$ voxel resolution over all folds of a leave-one-out cross validation.}
\end{figure}
...
ggplot(wa.n.all.vols, aes(x=mean, y=diff)) + 
  ...
  theme(legend.direction = "horizontal", legend.position = "bottom", 
        ...
        axis.text = element_text(size = 8))
@
  \caption{Bland-Altman plots across all folds of Winterburn atlas cross validation.}
\end{figure}
