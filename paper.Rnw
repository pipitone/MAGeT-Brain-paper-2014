...
bland_altman_plot <- function(x, y, c) {
  data = data.frame(x=x,y=y,diff=x-y, c=c)
  return(ggplot(data, aes(x= x, y = diff, colour = as.factor(c))) + 
...
the gold standard to compare with.  The segmentation accuracy reported is averaged over the ten validation
...
\begin{figure}
<<ADN1-bland-altman, cache=TRUE, dependson='ADNI1-Xval-prep'>>=
...
bland_altman_plot(a2a.best$volume.snt, a2a.best$volume.ma, a2a.best$DX) + geom_smooth(method="lm")
@
\end{figure}
...
subregions show distinctly larger \todo{what?} than resampling error. 
...
\begin{figure}
  \begin{centering}
    \includegraphics[width=6in]{figure/FEP-montage.pdf}
  \end{centering}
  ...
  resolution), the downsampled Winterburn segmentations (at 0.9mm3-isotropic
  voxel resolution), and the MAGeT brain labels on the downsampled atlas image.}
  \label{fig:FEP-montage}
\end{figure}
...
which are consistently larger \todo{ADD AN ESTIMATE OF THE DIFFERENCE HERE} than
manually segmented volumes in each disease category.  
...
segmentations for larger hippocampi \todo{WHEN COMPARED TO SNT SEGMENTATIONS} (Figure \ref{fig:ADNI1-src-Bland-Altman}.

Figure \ref{fig:ADNI-scr-subfields} shows the subfield volumes for the 69 ADNI1
...
qplot(DX,value,data=melt(totals.complete,measure.vars=c("FS", "FSL", "MAPER","MAGeT","SNT")),
      ...
      #geom_text(aes(y=Inf,x=interaction(DX,Method), vjust=2, label=pearson, colour=Method), size=3, data=correlations) +
      xlab("Diagnosis") + 
...
df = totals.complete
levels(melted) = c("FS" = paste("FSL", round(cor(df$SNT, df$FS),2)),
                   "FSL"= paste("FSL", round(cor(df$SNT, df$FSL),2)),
                   "MAPER"= paste("MAPER", round(cor(df$SNT, df$MAPER),2)),
                   "MAGeT"= paste("MAGeT", round(cor(df$SNT, df$MAGeT),2)))
...
ggplot(melted, aes(x= SNT, y = diff, colour=DX)) + 
