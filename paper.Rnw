...
taken from the ADNI Alzheimer's disease imaging database, and the second is a
5-fold validation over 81 subjects having had a first episode of psychosis. In
both cases, automated segmentations are compared with manual segmentions
...
as atlases we use five additional first episode psychosis subjects and
...
A maximum similarity score of \Sexpr{snt.mb.kappa.best} is found when using 9
...
See Section \ref{adnidataset} for study details, inclusion criteria and imaging
characteristics. 
...
images and segmentations. A template library of 19 images were randomly selected
from the target dataset of ADNI subjects, and \ants{} registration and majority
vote label fusion were used as these were found to perform favourably in earlier
experiments. 
