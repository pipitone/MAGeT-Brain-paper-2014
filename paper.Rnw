...
\usepackage{algorithm, color, algpseudocode, amsmath, lrl, geometry, ctable}
...
ggplot(data=fep.volumes, aes(x=manual, y=volume, colour=method)) + 
  ...
  ylab(expression("Mean bilateral computed hippocampus volume "  (mm^3))) + 
  scale_size_continuous(guide="none") 
@
...
\begin{figure}
<<fig:FEP-Bland-Altman, cache=TRUE, dependson='ADNI-volumes-prep', fig.width=7, fig.height=7>>=
...
measured in the FEP dataset by \snt{} segmentation and each of the
