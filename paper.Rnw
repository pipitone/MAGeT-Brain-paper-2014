...
these labels into a definitive segmentation for the target. 
...
cuts or EM approach with intensity spatial prior models to optimise the fused
segmentations.  Using images from the ADNI1 baseline dataset and 30 atlases. 

{\em Atlas selection} techniques are often used to filter out atlases dissimilar
to a target image in order to reduce {conceptual} errors \citep{Aljabar2009}.
...
manually labelled atlases and successively incorporating unlabelled target
images after being labelling using multi-atlas techniques\citep{Wolz2010}.  The
sequence in which target images are labelled is chosen so that the similarity
...
neuroanatomical features that match the target, and that the atlas segmentation
...
will refer to this as {\em basic multi-atlas segmentation}. The basic schema as
...
segmentation schema. Additional stages construct the template library by
labelling a subset of target images, rather than by using the atlas library.
The choice of targets used in the template library can be made to reflect the
neuroanatomy or demographics of the target set as a whole (for instance, by
sampling equally from cases and controls). A truncated version of basic
multi-atlas segmentation is used to label the template library images; label
fusion is not performed. Instead, each template image gets a labelling from each
atlas image. A second round of basic multi-atlas segmentation uses the template
library to segment the entire set of target images (including those images used
in the template library). Label fusion in this final step fuses all labels from
all templates.  Figure \ref{alg:MAGeT} describes the MAGeT brain algorithm in
pseudocode.  
...
Before registration, all images undergo preprocessing with the N3 algorithm
...
this paper we explore three different fusion methods. 
...
The Dice similarity index (SI) assesses the agreement between two segmentations.
It is one of the most widely used measures of segmentation performance, and we
use it as the basis of comparison throughout this paper.  Additionally, we
report other commonly used measures:

\begin{itemize}
 \item \begin{equation*}Similarity index (SI) =\frac{2|A \cap B|}{|A| + |B|}\end{equation*}
 \item \begin{equation*}Jaccard (J) =\frac{2|A \cap B|}{|A| + |B|}\end{equation*}
