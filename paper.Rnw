...
\usepackage{fancyhdr}
\pagestyle{fancy}
\fancyhead[RO,RE]{draft - January, 2014}
...
\newcommand{\snt}{SNT}
\newcommand{\expadnixval}{Whole Hippocampus Segmentation Cross-Validation --- Alzheimer's Disease} 
\newcommand{\expfepxval}{Whole Hippocampus Segmentation Cross-Validation --- First Episode of Psychosis} 
\newcommand{\expadni}{Whole Hippocampus Segmentation Comparision --- ADNI1 Complete 1Yr} 
\newcommand{\expsubfield}{Hippocampal Subfield Segmentation Cross-Validation} 
\newcommand{\expadnixvalsnt}{Whole Hippocampus Segmentation Cross-Validation --- Alzheimer's Disease, \snt{} Segmentations}
...
\begin{enumerate}
\itemsep-0.5em
\item check tense: we are is have has use/used find/found
\item remove 'accuracy' unless specifically talking relative to ground truth (and
    even then, qualify)
\item change overlap/similarity --> agreement? choose one. 
\item check that all chart labels are sensible 
\item check that Experiment 5 matches Experiment 1
\item check that all Experiment references resolve correctly
\item fix figure 2 schematic language for the experiments (not 2a 2b). 
\item ADNI-1 or ADNI1: standardize
\item check that experiment references are formatted as Experiment 2 (Section 3.2)
\item subfield or subregion -- choose one
\end{enumerate}}
...
limiting the number of inputs used during segmentation, and so demonstrating that
reliable application of the atlases' subfield segmentation to an unlabelled
target image is indeed possible using this methodology.
...
the temporal lobe. Manual input is only needed to mark mark divisions between
...
atlas and template library sizes, registration and label fusion methods. This
experiment is then replicated with a first episode psychosis patient dataset
to determine how the behaviour of \mb{} changes when segmenting younger and
differently diseased subjects, and to ensure that we have not overfit our
parameters to the aging or neurodegenerative brain. A leave-one-out validation
of subfield segmentation is performed on the \citet{Winterburn2013} manually
segmented high-resolution MR atlases, using the optimal \mb{} parameters found
from the previous experiments. Finally, we validate our algorithm using
all of the data available in the \adnidataset{} sample and compare \mb{}
...
\begin{itemize}
\item Experiment 1 investigates the accuracy of \mb{} whole hippocampus
segmentation of aging and Alzhiemer's diseased subjects over a wide range of
...
\item Experiment 2 consists of a similar cross-validation design to verify these
parameter settings on brain images of young, first episode psychosis
patients. 
\item Experiment 3 compares \mb{} whole hippocampus segmentations with those of
well-known automated methods on the entire \adnidataset{} image dataset consisting
of 
\item Experiment 4 assesses hippocampal subfield segmentation quality in a
leave-one-out cross-validation design. 
\Sexpr{length(means.complete$RID)} brain images of subjects diagnosed as
cognitively normal, having mild cognitive impairment, or Alzheimer's disease.
\end{itemize}
...
%%%%%                                                                     %%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Experiment 1: \expadnixval}
\label{exp:adni-xval}

\textbf{[TODO: cut, re-introduce as above]}
...
\textbf{[TODO: fix wording to relate to pruessner labels]}
Note, in the Supplementary Materials we have replicated this experiment using the
\snt{} semi-automated labels included with the ADNI dataset.

\subsubsection{Materials and Methods: \expadnixval}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Results: \expadnixval}
...
strategy did not significantly improve segmentation accuracy, contrary to the
...
<<fig:ADNI1-xval-k-mean,dependson='ADNI1-xval-load',fig.show='hide'>>=
...
<<fig:ADNI1-xval-variability,dependson='ADNI1-Xval-prep',fig.show='hide'>>=
...
\begin{figure}
  ...
  \caption{Dice's similarity score vs. atlas and template library size.  Dice's
  similarity score of \mb{} segmentations and \psegs{} for 60 ADNI subjects over a
  range of atlas and template library size, registration algorithm, and label
  fusion method on 10 folds of Monte Carlo cross validation.  Error bars indicate
  standard error.} 
...
\begin{figure}
  ...
  \begin{subfigure}[t]{0.48\textwidth}
    ...
    \caption{Difference in variability} 
    \label{fig:ADNI1-xval-variability}
  \end{subfigure}
  \caption{\textbf{Whole hippocampus segmentation cross-validation on
  Alzheimer's diseased subjects.}
(\ref{fig:ADNI1-xval-k-diff}) shows increase in mean Dice's Similarity Coefficient of \mb{} over
  \multiatlas{} segmentations vs. atlas and template library size when using the
  \ants{} registration method, and majority-vote label fusion over 10 folds of
  Monte Carlo cross validation.  Segmentation similarity is computed against
  \psegs{}.  Error bars indicate standard error.
(\ref{fig:ADNI1-xval-variability}) compares the variability in Dice scores of \mb{} vs.
  \multiatlas{} segmentations in the validation.  Variance of segmentation
  accuracy between \mb{} and \multiatlas{} segmentation is computed for each
  subject across over 10 folds of Monte Carlo cross validation.  Shown on
  the y-axis is the p-value resulting from a t-test comparing the distribution of
  variances at each parameter setting (atlas/template library size).  Only points
  where \mb{} mean variability is lower than \multiatlas{} are shown.  Dashed
  lines indicate a p-value of 0.05 and 0.01.}
\end{figure}
...
%%%%%                                                                     %%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Experiment 2: \expfepxval}
\label{exp:fep-xval}
\textbf{[TODO: tie with exp 1]}
...
whole hippocampal segmentations of a dataset of patients having had a single
episode of psychosis. The resulting segmentations are assessed for quality by
comparison with expert manual segmentations.

\subsubsection{Materials and Methods: \expfepxval}
\paragraph{First Episode Psychosis Dataset}
...
%%%%%                       of psychosis patients                         %%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Results: \expfepxval}
\textbf{[TODO: move up results to here.]}
 
% In this experiment \mb{} is applied to a dataset of images of first episode
% psychosis patients, using the Winterburn atlases and a template library of
...
<<fig:FEP-Bland-Altman,dependson='fig:FEP-vols',fig.width=7,fig.height=3,fig.show='hide'>>=
...
Manual segmentation of all 81 subjects is completed using the \prsnr-protocol.
\mb{} is configured using \ants{} registration algorithm and majority vote label
fusion. 
(\ref{fig:FEP-xval}) shows mean DSC overlap similarity score of \mb{}
  segmentations as they vary with atlas and template library size over 5-fold
  validation. Error bars indicate standard error. 
(\ref{fig:FEP-vols}) shows the volumes of the 81 subjects as determined by \fsl,
  \freesurfer, and \mb{} configured in two ways: with an atlas library
  of the five Winterburn atlases (MAGeT-Winterburn), or with an atlas
  library of five manually segmented FEP subjects as atlases (MAGeT-FEP). Linear
  fit lines are shown, with the shaded region showing standard error.
(\ref{fig:FEP-Bland-Altman}) shows the agreement between computed volumes and
  manually segmented volume. The overall mean difference in volume, and limits of
...
%%%%%                                                                     %%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Experiment 3: \expadni}
\label{exp:adni}
...
\subsubsection{Materials and Methods: \expadni}
\paragraph{\adnidataset{} dataset} 
...
atlases (described in Experiment 4, section \ref{exp:winterburn-xval}), and size of template library,
registration method, and label fusion method were determined by the optimal
settings found in Experiment 1 (Section \ref{exp:adni-xval}).  The \fsl{} segmentation method was used via
...
\paragraph{Evaluation method}
As in previous experiments, the \snt{} and Winterburn hippocampal segmentation protocols
differ in the neuroanatomical features delineated, and so we assess \mb{} by
...
%%%%%                       of ADNI subjects                              %%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Results: \expadni}

Based on the results from Experiment 1 (Section \ref{exp:adni-xval}), in this
experiment \mb{} was configured with a template library of 21 randomly chosen
subject images (7 from each disease class) and majority vote label fusion.  The
entire \adnidataset{} dataset was segmented by \mb{}, and the resulting volumes
compared with those obtained by expert semi-automated segmentation (\snt{}), and
three other automated segmentation techniques: \maper{}, \freesurfer{}, and
\fsl{}. Table \ref{tab:ADNI1-1.5T-Complete-1Yr-Dataset-Method-Totals} shows the
total count of segmentations available, including a count of those which have
failed a quality control inspection.  A total of
\Sexpr{length(means.complete$RID)} images are included in the following
analysis, having met quality criteria and having segmentations from every
method. 
...
\textbf{[TODO: combine volume plot/box-plot into a single figure]}
...
\begin{figure}
  ...
  in Experiment 1 (Section \ref{exp:adni-xval}. Sagittal slices are shown for each unlabelled T1-weighted
...
%%%%%                                                                     %%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Experiment 4: \expsubfield}
...
\subsubsection{Materials and Methods: \expsubfield}
...
voxel images to simulate standard 3T T1-weighted resolution images. Image
subsampling is performed using trilinear subsampling techniques.  In each round
of LOOCV, a single atlas is selected and segmented with \mb{} once using each
possible triplet of atlas images from the remaining four unselected atlases.
Thus, a total of ${3 \choose 4}=4$ segmentations are evaluated for each atlas
image, for $5\times4=20$ segmentations evaluated in total.  We chose an atlas library
with an odd number of images so as to ensure an unbiased label fusion when using
majority voting (see Discussion). 

The template library used has a total of 19 images composed of all $0.9mm^3$
voxel target images plus an additional set of 14 of healthy subject 3T T1 images
acquired separately (Table \ref{tab:WAval-healthy-demographics}). These images
were acquired on a 3T GE Discovery MR 750 system (General Electric, Milwaukee,
WI) using an 8-channel head coil with the enhanced fast gradient recalled echo
3-dimensional acquisition protocol, EFGRE-BRAVO, with the following parameters:
...
\textbf{[TODO: "What is the full voxel resolution of the EFGRE-BRAVO
sequence?"]}
...
can evaluate the reliability, or {\it precision}, with which \mb{} produces
hippocampal subfields that correspond to the segmentation protocol used by the
given atlas library images. By directly resampling the Winterburn atlas
segmentations to $0.9mm^3$ voxels (using standard nearest-neighbour image
resampling techniques) we obtain a scaled down version of the labels which
preserve the original segmentation protocol within the limits of error from
rounding and interpolation. Therefore, using the resampled Winterburn
segmentations as definitive for the $0.9mm^3$ resolution we evaluate reliability
of \mb{} segmentations using using DSC overlap scores and evaluate consistency
across the range of hippocampal sizes using Bland-Altman plots of subfield
volumes. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Results: \expsubfield}
...
@
  \caption{Similarity of \mb{} segmentation of subregions and the resampled
  Winterburn atlas segmentations at $0.9mm^3$ voxel resolution, over all folds of a
  leave-one-out cross validation. Overlap score for each hemisphere is measured
  separately. \label{fig:WAxval-kappa}}
\end{figure}
...
subregion across all subjects and folds of the validation. Mean and standard
deviation DSC scores of the subregions are shown in Table
...
that the CA4/DG subregion shows the highest mean DSC score of
\Sexpr{mean(CA4DG)} $\pm$\Sexpr{sd(CA4DG)}, followed by the Subiculum and CA1
subregions having scores of \Sexpr{mean(CA1)} $\pm$\Sexpr{sd(CA1)} and
...
The whole hippocampus is segmented with a mean DSC score of\Sexpr{mean(Whole)}
$\pm$\Sexpr{sd(Whole)}.

\begin{figure}
<<fig:WAxval-Bland-Altman,fig.height=5,fig.width=7>>=
...
ggplot(wa.n.all.vols, aes(x=mean, y=diff)) + 
  ...
  xlab(expression('Mean of resampled and MAGeT-Brain subregion volume ' (mm^3))) + 
  ylab(expression('Difference of manual and MAGeT-Brain subregion volume ' (mm^3))) + 
  ...
  theme(legend.direction = "horizontal", legend.position = "bottom", 
        ...
        axis.text = element_text(size = 8))
@
  \caption{\textbf{Bland-Altman plots comparing unilateral hippocampal subregion volume
across all folds of Winterburn atlas cross validation.} The overall mean
difference in volume, and limits of agreement ($\pm 1.96SD$) are shown by dashed
horizontal lines. Linear fit lines are shown. Note, points below the mean
difference indication overestimation of the volume with respect to the resampled
volume, and vice versa. \label{fig:WAxval-Bland-Altman}}
\end{figure}
...
translated by one voxel (i.e.: $0.9mm$ in all directions), scaled by 0.99, and scaled
by 1.01 (representing a 1\\% shrinkage or scale in all three cardinal directions).
Values are given as mean Dice's Kappa coefficient $\\pm$ stddev."
...
manual segmenation labels as ground truth in computing DSC scores. AD =
...
\textbf{[TODO: fix experiment references]} 
...
to provide accurate and reliable segmentations.  Experiment 1 (Section
\ref{exp:adni-xval}) compares \mb{} to basic-\multiatlas{} segmentation by
characterising the behaviour of each approach with varying parameter settings,
and allows us to choose optimal settings for subsequent experiments.  Experiment
2 (Section \ref{exp:winterburn-xval}) emonstrates the reliability of \mb{} in
producing subfield segmentations which match the segmentation protocol of the
input atlases despite contrast and resolution limitations in standard
...
of these methods and \mb{}. The overlap DSC scores for \mb{} subregions are
...
Nevertheless, the advantage of evaluating \mb{} on standard 3T T1-weighted
resolution MR images with a publically available atlas library is that our
results reflect typical usage scenarios of researchers and clinicians. 
\textbf{[TODO: mention that DSC by it's nature penalizes structures with high surface
area-to-vol ratios, so necessarily penalise all subfield errors.]}
\textbf{[TODO: discuss perturbed segmentation kappa values in relation to MAGeT
error]}

Our overlap DSC values demonstrates that we can reliably reproduce segmentations
for the CA1, subiculum, and CA4/dentate subregions.  That the CA2/CA3 and
molecular layers are less well reproduced (DSC < 0.5) should not be surprising
as these are extremely thin and spatially convoluted regions that originally
required high-resolution MRI for identification and so it is likely that the
extents of these regions are well below the resolution and contrast offered by
...
segmentation methodologies. A further complication is that different
...
resolved.  Thus, because of these disagreements and the difficulting discerning
subregions in standard T1-weighted images, we opted to speak of the precision or
reliability of \mb{} subfield segmentation rather than speak of accuracy.  
...
\FloatBarrier
\subsection{Experiment 5: \expadnixvalsnt}
...
\subsubsection{Materials and Methods: \expadnixvalsnt}
\paragraph{Dataset} 
...
% ######################
\subsubsection{Results: \expadnixvalsnt}
...
ggplot(subset(stats.tests, direction > 0 & stat == "Variance"), 
  ...
  theme(legend.direction = "horizontal", legend.position = "bottom")
@
  \caption{\textbf{Difference in variability of \mb{} vs. \multiatlas{} segmentation
  with respect to \snt{} segmentation.}
  Variance of segmentation accuracy between \mb{} and \multiatlas{} segmentation
  is computed for each subject across over 10 folds of Monte Carlo       
  cross validation.  
  Shown on the y-axis is the p-value from a t-test comparing the distribution of
  variances at each parameter setting (atlas/template library size).  Only points
  where \mb{} mean variability is lower than \multiatlas{} are shown.  Dashed
  lines indicate a p-value of 0.05 and 0.01.}
