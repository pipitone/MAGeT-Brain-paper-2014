...
of the final transformation \citep{Avants}. The transformation is estimated in a
...
nonlinear registration methods such as \ants \cite{Avants} sufficiently filters
...
are not simply a smoothing or averaging effect \cite{Chakravarty2013}.  
...
\cite{Warfield2004} (or variants of the STAPLE algorithm \cite{stuff from the
Duchesne group here} which weights each segmentation based upon its estimated
...
is worth noting that this limitation was previously identified by Heckemann et
al. \cite{Heckemann2006a} and, subsequently, other groups have not even
considered the potential pitfalls of an even number of candidate labels (eg.
\cite{Leung2010}).
...
have only ever performed a 3-fold validation \cite{????}.  This suggests that
...
\todo{additionally, we could be seeing a ceiling effect: reaching the limit of
what is achievable with a multi-atlas-based segmentation method}
...
implementation with the Winterburn atlases \cite{Winterburn2013}.  Amongst these
...
misregistrations in candidate segmentation (i.e. segmentations generated that are
then input in the voxel-voting procedure \cite{Collins2010}). The work of Leung
\cite{Leung2010} (work also reports some of the highest kappas for the
segmentation of ADNI data) tackles this problems by using an intensity threshold
that is estimated heuristically at the time of segmentation.  While this works
for the ADNI dataset (which is partially homogenized with respect to image
acquisition and pre-processing), it is unclear if this type of heuristic is
applicable to other datasets.  In all cases, these methods require more atlases
than our implementation with the Winterburn atlases.  Lotjonen et al.
\cite{Lotjonen2010} achieve highly accurate segmentation but correct their
segmentations using classifications derived using an expectation maximization
framework.  In their initial work, Chupin and colleagues develop their
...
increased computational cost ($\Omicron(log(n))$), as previously mentioned in
other work \cite{Heckemann}). 

In comparison to the methods that we compared and were available through the
ADNI database (FreeSufer, MAPER) and the method that we initialized ourselves
...
the ADNI database.  However, FreeSufer and FIRST provide radically different
...
\cite{Yushkevich2009, and VanLeemput2009}.  However, the FreeSufer implementation of
...
no explicit validation of the methodology based on accruacy or precision.
