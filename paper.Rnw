...
experiment (\expadnixvalsnt{}, Supplementary Materials) where atlases use a different
...
(see \expfepxval{}) ignoring subfield delineations.  All images from the
first episode dataset are segmented by \mb{}.  The optimal size of template
library, registration method, and label fusion method found in \expadnixval{} are
used. 
