...
%  - ADNI-1 or ADNI1, standardize
%  - Don't use "Experiment X", instead use some short form description, and then
%  reference (Experiment X). 
%  - subfield or subregion -- choose one
...
% TODO: fix wording, relate to pruessner labels
...
similarity score of \Sexpr{mb.kappa.best} is found when using 9
...
\begin{figure}
<<fig:ADNI1-xval-k-mean,dependson='ADNI1-xval-load'>>=
...
@
  ...
  size, registration algorithm, and label fusion method over 10 folds of Monte Carlo       
  cross validation. 
...
\begin{figure}
<<fig:ADN11-xval-k-diff,dependson='ADNI1-xval-load'>>=
...
@
  ...
  \ants{} registration method, and majority-vote label fusion over 10 folds of Monte 
...
\begin{figure}
<<fig:ADN1-Xval-variability,dependson='ADNI1-Xval-prep'>>=
...
\begin{figure}
  ...
  \begin{subfigure}[t]{0.48\textwidth} 
    ...
    \caption{Mean DSC overlap score vs. atlas and template library size in
    a 5-fold Monte Carlo cross-validation (MCCV) of \mb. Error bars
  ...
  \begin{subfigure}[t]{0.48\textwidth}
    ...
    \caption{Computed vs. manually segmented mean hippocampus volume. Linear fit
      lines are shown, with the shaded region showing standard error. }
    \label{fig:FEP-vols}
  \end{subfigure}
  ...
  \end{subfigure}
  \caption{\textbf{First Episode Patient dataset automated segmentation.}
...
fusion. In Figure \ref{fig:FEP-vols} \mb{} is run both with an atlas library
consisting of the five Winterburn atlases (MAGeT-Winterburn), or with an atlas
library of five manually segmented FEP subjects as atlases (MAGeT-FEP).}
\end{figure}
...
through a leave-one-out validation in which resampled images ($0.9mm^3$ voxels)
of each Winterburn atlas subject are segmented using the remaining
high-resolution Winterburn atlases. 
...
in context and compared to existing (semi-)automated methods (see Discussion). The whole hippocampus is segmented with a mean DSC score of\Sexpr{mean(Whole)} $\pm$\Sexpr{sd(Whole)}.
...
Based on the results from Experiment 1 (Section \ref{exp:adni-xval}), in this experiment \mb{} was configured
...
(CN); LOOCV = Leave-one-out cross-validation; MCCV = Monte Carlo
...
\ref{exp:adni-xval}) compares \mb{} to
basic-\multiatlas{} segmentation by characterising the behaviour of each
approach with varying parameter settings, and allows us to choose optimal
settings for subsequent experiments. 
%TODO: fix refs 
Experiment 2 (Section \ref{exp:winterburn-xval}) emonstrates the reliability of
...
T1-weighted image volumes. 

Experiments 2 and 3 validate whole hippocampal
...
%TODO: mention that DSC by it's nature penalizes structures with high surface
%area-to-vol ratios, so necessarily penalise all subfield errors.
%TODO: discuss perturbed segmentation kappa values in relation to MAGeT error
