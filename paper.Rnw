...
ggplot(all_data, aes(x=templates.y,y=k.y, colour=as.factor(atlases))) + 
...
ggplot(subset(all_data, reg_method=="ANTS" & method.y=="Majority Vote"), 
  ...
  #coord_cartesian(ylim=c(0,0.07), xlim=c(0.5,10.5))
@
...
attach(combined)
totals = data.frame(RID = RID, VISCODE = VISCODE, DX = DX.bl,
...
qplot(DX,value,data=melt(complete_totals,measure.vars=c("FS", "FSL","UCSD", "MAPER","MAGeT","SNT")),
...
\begin{figure}[h]
<<ADNI-scr-histogram,dependson='ADNI-baseline-volumes-boxplot',cache=T>>=
melted   = melt(complete_totals, measure.vars=c( "SNT","FSL","FS","MAPER","MAGeT"))
binwidth = 250
...
ggplot(data=melted, aes(x=value)) + 
  ...
  geom_text(aes(y=25,x=label_x_pos,hjust=1,size=1, 
                label=paste("sigma ==",signif(sd, digits=3))), parse=TRUE,
            data=testsdf) + 
  geom_text(aes(y=21,x=label_x_pos,hjust=1,size=1, 
                label=paste("mu ==",signif(mean, digits=3))), parse=TRUE,
            data=testsdf) + 
  xlim(0, 10000) + ylim(0,30) + 
  facet_grid(variable ~ DX) + 
  scale_colour_discrete(legend=FALSE) + 
  scale_size_continuous(legend=FALSE) + 
...
@
  \caption{{\bf ADNI Baseline cohort.} Comparison of total hippocampal volumes as measured by SNT, FSL, FreeSurfer (FS), MAPER, and MAGeT brain (MAGeT). A fitted normal curve is shown in red. $W$ is the Shapiro-Wilkes test statistic measuring normality of the data (low p-values indicate non-normality).}
  \label{ADNI-scr-histogram}
\end{figure}


\begin{figure}[h]
<<ADNI-src-volumes-plot,echo=F,fig=T, fig.align='center',dependson='ADNI-baseline-volumes-boxplot',cache=T>>=
by_snt_volumes = melt(totals, measure.vars=c( "SNT","FSL","FS","MAPER","MAGeT"))

with(by_snt_volumes,
ggplot(data=by_snt_volumes, aes(x=value)) + geom_histogram(binwidth=250) +
    facet_grid( variable ~ DX) 
     )
#     geom_text(aes(
...
melted = melt(totals, measure.vars=c("FSL","MAPER","SNT", "FS", "UCSD"))
cor_by_dx    = by(melted[c("MAGeT", "value")], c(melted['variable'], melted['DX']), 
                  function(x) cor(x$MAGeT,x$value, use="pairwise.complete.obs"))
cor_overall  = by(melted[c("MAGeT", "value")], melted['variable'], 
                  function(x) cor(x$MAGeT,x$value, use="pairwise.complete.obs"))
cor_by_dx = t(rbind(cor_by_dx))
cor_overall = rbind(cor_overall)

vol_by_dx = t(rbind(by(totals$MAGeT, totals$DX, mean)))
colnames(vol_by_dx) <- "Mean Volume"
vol_overall = matrix(mean(totals$MAGeT),dimnames=list("Overall"))

dx      = cbind(vol_by_dx, cor_by_dx)
overall <- cbind(vol_overall, cor_overall)

latex(format.df(rbind(dx, overall), dec=2),
      n.cgroup=c(1,5),cgroup=c("","Volume Correlation"),
      n.rgroup=c(4,1), file="", size="scriptsize")
...
gold_diff     = (gold_0.3mm_total - gold_0.6mm_total)/gold_0.3mm_total
mb_bravo_diff = (gold_0.3mm_total - mb_bravo_total)/gold_0.3mm_total
mb_0.6mm_diff = (gold_0.3mm_total - mb_0.6mm_total)/gold_0.3mm_total

subjects = gold_0.6mm[,1]
mb0.6mm_diff_over_gold_diff = cbind(subjects, mb_0.6mm_diff - mean(gold_diff) , res="diffdff")
gold_diff     = cbind(subjects,gold_diff, res="0.3mm gold & 0.6mm gold")
mb_bravo_diff = cbind(subjects,mb_bravo_diff, res="0.3mm gold & MAGeT-on-BRAVO")
mb_0.6mm_diff = cbind(subjects,mb_0.6mm_diff, res="0.3mm gold & MAGeT-at-0.6mm")


diff = rbind(mb_bravo_diff,mb_0.6mm_diff,gold_diff, mb0.6mm_diff_over_gold_diff)
names(diff) <- c("subject", "CA1", "Subiculum", "CA4", "CA2/CA3", "Dentate", "res")

melted = melt(diff, id.vars=c("subject", "res"))
names(melted) <- c("subject","res", "region","voldiff")
ggplot(melted, aes(y=voldiff,x=region)) +
  geom_boxplot() + facet_grid( . ~ res)  + 
  labs(title="Percent difference in MAGeT volumes from original",
       y="Percent difference in volume", 
       x="Region") 
@
