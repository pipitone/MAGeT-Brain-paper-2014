...
In the Supplementary Materials we have replicated this experiment using the \snt
semi-automated labels included with the ADNI dataset.
...
(+\Sexpr{round(snt.mb.diff.best, digits=3)} DSC) is found with a single atlas
...
subjects and validation folds (Figure \ref{ADNI1-xval-variability}). A
...
<<WAval-prep, cache=T>>=
wa.xval.vols = subset(read.csv('data/WAval.csv'))
wa.xval.kappa = subset(read.csv('data/WAval-kappa.csv'))
...
\begin{figure}[h!]
<<WAval-bland,cache=T,fig.width=7,fig.height=6>>=
wa.xval.volumes = melt(wa.xval.casted, id.vars=c('file', 'region', 'gold_0.3mm'))
wa.xval.volumes = rename(wa.xval.volumes, c("variable"="measure", "value"="volume"))
levels(wa.xval.volumes$measure) <- list("Subsampled atlas labels"="gold_0.9mm",
                               "MAGeT on subsampled atlas images" = "mb_0.9mm",
...
wa.xval.volumes$diff = wa.xval.volumes$gold_0.3mm - wa.xval.volumes$volume 
wa.xval.volumes$mean = (wa.xval.volumes$gold_0.3mm + wa.xval.volumes$volume)/2
...
ggplot(wa.xval.volumes, aes(x=mean, y=diff, colour=measure)) + 
  ...
  xlab(expression('Mean of manual and automated HC volume ' (mm^3))) + 
  ylab(expression('Difference of manaul and automated HC volume ' (mm^3))) + 
  scale_color_discrete(name="") + 
  theme(legend.direction = "horizontal", legend.position = "bottom", 
...
\mb{}                   &9 & \Sexpr{round(mb.kappa.best, digits=3)} & & 10
...
the cross validation conducted in Experiment 1.  We find that both increasing
the number of atlases and the number of templates used improves \mb{}
...
generating a template library reduces the variability in segmentations (i.e.
\mb{} more consistently produces high quality segmentations than does
basic-\multiatlas). 
...
(Figure \ref{fig:ADNI-segmentations}). Although the \snt{} segmentations are
used as benchmarks for validation in many other studies (Table
...
segmentations (Experiment 5) shows noticeably poorer mean similarity scores for
both \mb{} and \multiatlas. 
...
demonstrates that we can reliably produce CA1, subiculum, and CA4/dentate
subfield segmentations that correspond to those in the input atlas with only 
only modest amounts of error in volume.  The fact that CA2/CA3 and molecular layers
cannot be reasonably reproduced should not be surprising as these are extremely
...
In addition, some manual subfield segmentation methodologies do not attempt to parse
these regions even on high-resolution scans (\citep{Wisse2012, Mueller2009}; see
also Table \label{tab:subfield-segmentation-protocols}). Note, it is because of
both these reasons (the inability to discern subregions in standard T1-weighted
images, and the definitional uncertainty in subfield segmentation at the present
moment) that we opted to speak of the precision or reliability of \mb{} subfield
segmentation rather than speak of accuracy. 
