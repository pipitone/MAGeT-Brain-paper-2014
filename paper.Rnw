...
\psegs using Dice's Similarity Coefficient (DSC). 
...
\mb{}-based segmentations using
both \animal{} and \ants{} registration algorithms demonstrate good agreement with \snt{}
segmentations (maximum mean DSC of \Sexpr{round(snt.mb.kappa.best, digits=2)}
...
With at least five templates, \mb{} shows a mean improvement in similarity score
over \multiatlas{}-based segmentation for the same size of atlas library with
majority vote label fusion (Figure \ref{fig:ADNI1-SNT-xval-k-diff}). The
magnitude of improvement increases with the size of template library, and shows
diminishing returns with larger atlas libraries. Peak improvement
(+\Sexpr{round(snt.mb.diff.best, digits=3)} DSC) is found with a single atlas
and template library of 19 images.
...
In addition to a mean increase in similarity score over \multiatlas{}-based
segmentation, \mb{} also shows more consistency in similarity scores across all
subjects and validation folds (Figure \ref{ADNI1-SNT-xval-variability}). A
template library of at least 13 images is sufficient to show significant ($p <
0.05$) decrease in variance for all sizes of atlas library tested (1-9 images).  
