...
all_data$k_diff = all_data$k.x - all_data$k.y
...
ggplot(all_data, aes(x=templates.y, y=k_diff)) + 
  facet_grid(reg_method~method.y) + 
  stat_summary(fun.y=mean,geom='bar',position="dodge",
               aes(y=k_diff,fill=as.factor(atlases))) + 
...
\begin{figure}
<<ADN1-Xval-k_diff-ANTS-20_tmpls, cache=TRUE>>=
...
@
  \caption{Difference in mean Kappa between multi-atlas and MAGeT brain using ANTS, 20 templates}
  \label{}
\end{figure}
