...
\todo{Nomenclature: SFEP Dataset}
\todo{describe image characteristics}
\todo{describe expert segmentations}
SFEP dataset demographics are shown in table \ref{tab:SZFEP_Demographics}. 
