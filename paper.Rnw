...
in table \ref{tab:ADNI1_1.5T_Complete 1Yr_Dataset_Demographics}. 
...
latex(tab, file="", size="scriptsize", 
    caption="ADNI1 1.5T_Complete_1Yr dataset demographics",
    title="tab:ADN1_1.5T_Complete_1Yr_Dataset_Demographics")
@
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
The following experiments were performed to assess the performance of \mb with
various parameter settings as well as on diverse datasets.  In each experiment
we contrast the performance of \mb with that standard single- and multi-atlas
...
the ADNI1 dataset for validation.  This form of cross-validation (described in
detail below) allows us to validate \mb with various atlas and template
...
ADNI1:Complete 1Yr 1.5T} standardized dataset. 23 subjects were chosen from each
...
{\bf Atlas and template libary.}  The atlas and template libraries are selected
from the dataset evaluated.  Atlases consist of images taken from the
dataset, with corresponding manual labels provided by SNT. Template library 
images were selected at random from remaining images not used as atlases. The
atlas library size ranged from 1 to 9 images. Template library size ranged from
1 to 20 images. 
...
random sub-sampling cross-validation, consists of repeated rounds of validation 
in which items from the dataset are randomly sampled (without replacement) and 
assigned to a training set or validation set \cite{Shao1993}.  
...
%%%%%
%%%%%               Experiments: ADNI-1 Complete 1Yr Validation               %%%%%
%%%%%
\subsubsection{ADNI-1 Complete 1Yr Validation}
...
{\bf Dataset evaluated.} All images from the {\em ADNI1:Complete 1Yr 1.5T}
...
{\bf Dataset evaluated.} To validate that \mb performance generalises to
across disease categories, we measure the performance of \mb on the
...
subjects are segmented in separate runs of \mb.  In each run, the subject
...
%                                                                              %
<<ADN1-xval-load, include=FALSE, cache=TRUE>>=
...
@
  ...
  \caption{
  {\em Mean DSC by atlas and template library size, registration algorithm,
...
and voting method (Figure \todo{fig:ADNI1-xva-k-diff}. The magnitude of improvement over
...
ggplot(stats.tests, 
  ...
  ylab( "p-value" ) 
@
  \caption{Difference in variability of \mb vs. multi-atlas segmentation accuracy. 
  In these plots we consider the segmentation accuracy within each subject across
  the 10 rounds of validation per parameter setting. Standard deviation and 
  variance is computed per subject for \mb and multi-atlas, and the distribution
  of these test statistics is compared with a t-test. The p-value of this test, 
  is shown on the y-axis (scaled logarithmically).}
...
\ref{ADNI1-xval-Bland-Altman}). \mb shows a fixed bias towards producing smaller
segmentations, and shows a negligble proportional bias towards smaller 
segmentations inversely proportional to volume size. Nevertheless, when using 
3 or more atlases, \mb volumes are within the 95\% limits of agreement with 
manual labels.

\begin{figure}
<<ADN1-xval-Bland-Atlman, cache=TRUE, dependson='ADNI1-Xval-prep'>>=

bland_altman_plot <- function(data, x, y) {
...
a_20_tmpls = subset(all_data, 
                      ...
                      method.mb    == "Majority Vote" &
                      templates.mb == 20 &
                      atlases      %in% seq(1,9,by =2) &
                      volume.ma     < 10000)
total_HC_volume = ddply(a_20_tmpls, c("subject", "label", "atlases"), function (df) {
  data.frame(
    ma = sum(df$volume.ma),
    mb = sum(df$volume.mb)
  )
})
total_HC_volume$diff = total_HC_volume$ma - total_HC_volume$mb

ggplot(total_HC_volume, aes(x=ma, y = diff, group=atlases)) + 
    geom_hline(yintercept = mean(total_HC_volume$diff) + c(-2, 0, 2) * sd(total_HC_volume$diff), 
               linetype=2, color='brown') + 
    geom_point() + 
  facet_grid( . ~ atlases) + 
  xlab('Mean multi-atlas and MAGeT Brain total Hippocampus volume') + 
  ylab('Multi-atlas - MAGeT Brain volume') 
@
  \caption{Bland-Altman plot comparing multi-atlas and \mb hippocampal volumes 
  when using 9 atlases, and a 20 image template library.}
...
#bl    = read.csv("data/ADNI_baseline_volumes/ADNI1_Screening_1.5T_11_15_2012.csv")
...
combined = merge(combined, qc          , by=c("Image.Data.ID"), all.x=TRUE)

# A table showing the number of images (and failures) from each package
#           Total  MAGeT  FSL  FS   MAPER
# Images    
# Failures
# 
# Total number complete images = 
...
all_qc = subset(all_qc, TEMPQC != "Fail" & MAGeT_QC == 1 | FSL_QC == 1 )
...
\Sexpr{length(totals.complete$RID)} images). 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% ADNI1 Screening Dataset Package Totals & Failures                           %
%                        
%
\begin{figure}
<<ADNI1-scr-package-totals, echo=F, results="asis",cache=FALSE>>=
latex(package_totals, file="", size="scriptsize", 
    caption="ADNI1 1.5T Complete 1yr Dataset Segmentations",
    title="",
    label="tab:ADN1-1.5T-Complete-1yr-Dataset-Segmentations")
@
  \caption{Pass/fail quality control indicators were supplied with the FreeSurfer
  ...
  quality inspection for MAGeT and FSL segmentations.  
  }
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
<<ADN1-FEP-Bland-Atlman, cache=TRUE, dependson='FEP-volumes'>>=
...
gold_0.9mm = subset(data, version == "gold_0.9mm")
mb_bravo   = subset(data, version == "mb_bravo")
mb_0.9mm   = subset(data, version == "mb_0.9mm")

gold_0.3mm_total = gold_0.3mm[,2:6] + gold_0.3mm[,7:11]
gold_0.9mm_total = gold_0.9mm[,2:6] + gold_0.9mm[,7:11]
mb_bravo_total   = mb_bravo[,2:6] + mb_bravo[,7:11]
mb_0.9mm_total   = mb_0.6mm[,2:6] + mb_0.9mm[,7:11]

# calculate percent error from the gold standard measured at 0.3mm resolution
pe_gold0.9mm = (gold_0.9mm_total - gold_0.3mm_total)/gold_0.3mm_total * 100
pe_mbbravo   = (mb_bravo_total - gold_0.3mm_total)/gold_0.3mm_total * 100
pe_mb0.9mm   = (mb_0.9mm_total - gold_0.3mm_total)/gold_0.3mm_total * 100

subjects = gold_0.9mm[,1]
pe_gold0.9mm = cbind(subjects,pe_gold0.9mm, res="Resampling to 0.9mm")
pe_mbbravo   = cbind(subjects,pe_mbbravo,   res="MAGeT-on-BRAVO")
pe_mb0.6mm   = cbind(subjects,pe_mb0.9mm,   res="MAGeT-at-0.9mm")
