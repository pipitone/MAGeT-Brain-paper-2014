...
labelled using multi-atlas techniques. The sequence in which target images are
...
attempt to define the hippocampus and its subfields using standard T1-weighted
data. The first is included with the FreeSurfer package \citep{VanLeemput2009}.
This work is limited as it omits the tail of the hippocampus and the
segmentation protocol has yet to be fully validated. Nonetheless, it
demonstrates that the applicability of hippocampal subfield segmentation using
data from 10 subjects. In the second method, \citet{Yushkevich2009} hippocampal
subfields were acquired and labelled on high-resolution MRI data from post-mortem
medial temporal lobe samples. Using nonlinear registration guided by manually
derived hippocampus masks and specific landmarks, the authors demonstrated
...
other publications \citep{Collins2010,Heckemann2011,Aljabar2009}.
...
resampling error. The five $300\mu$-isotropic voxel Winterburn atlas images and
...
as input subjects. Subsampling the subject images allows us to assess \mb in a
...
To validate that \mb works effectively in the context of other neuropsychiatric
...
@
  ...
  multi-atlas segmentation using the same number of atlases.
...
ggplot(data=melted, aes(x=SNT, y=volume, colour=Method)) + 
    geom_point(size=1) + 
    geom_smooth(method="lm") + 
    xlab(expression("SNT \\gs hippocampal volume " (mm^3))) + 
