...
derived from serial histological data \citep{MallarChakravarty2012} is possible.  The
...
\Sexpr{length(means.complete$RID)} images; Table 
...
Within disease categories (Figure \ref{fig:ADNI-volumes-boxplot}),
...
we constructed Bland-Altman plots for each method (Figure \ref{fig:ADNI1-Bland-Altman}).  
...
for instance).  What is most striking in Figure \ref{fig:ADNI1-Bland-Altman}
...
qplot(DX,value,data=m2,
      ...
      xlab("Diagnosis") + 
      ylab(expression(paste("Mean hemispheric hippocampal volume (", mm^3, ")"))) + 
...
ggplot(data=melted, aes(x=SNT, y=volume, colour=Method)) + 
    geom_point() + 
    geom_smooth(method="lm") + 
    xlab(expression("Manual mean hemispheric hippocampal volume " (mm^3))) + 
    ylab(expression("Automated mean hemispheric hippocampal volume " (mm^3))) +
...
\begin{figure}
<<ADN1-Bland-Atlman, cache=TRUE, dependson='ADNI-volumes-prep', fig.width=6, fig.height=6>>=
...
nonlinear registration methods such as \ants \cite{Avants2008} sufficiently filters
...
are not simply a smoothing or averaging effect \cite{MallarChakravarty2012}.  
...
\cite{Warfield2004} (or variants of the STAPLE algorithm \cite{Duchesne2012})
...
other work \cite{Heckemann2006}). \todo{check this: time complexity seems suspicious} 
...
\cite{Yushkevich2009,VanLeemput2009}. However, the FreeSufer implementation of
