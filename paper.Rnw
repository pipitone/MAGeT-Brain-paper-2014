...
neuropsychiatric disorders \citep{Mouiha2011b}.  The result is that existing
hippocampal atlases available to a researcher may not accurately represent
neuroanatomy of a specific population under study.  Additionally, in the course
of a research or clinical study, it may be necessary to make adjustments to
hippocampal definition as a means of hypothesis testing.  For example, Poppenk
...
difference of those regions and recollection memory performance.  Making
modifications to an set of atlas library segmentations may require the same
level of expertise and manual effort needed to create the segmentations from
scratch.  

%  Automated methods 
%
%  Heckemann2006 - 
%  Heckemann2011 -  Automatic morphometry in AD and MCI 
%    - (MAPER). multi-atlas. 30 atlases, entire ADNI screening  
%  Collins2010 - .. seg of HC and AM by augmenting ANIMAL
%    - multi-atlas 80 atlases, atlas selection (NMI)
%  Aljabar2009 - atlas selection and effect on accuracy
%    - 
%  Leung2010 - X-sectional and longitudinal HC vol in MCI, AD
%    - (MAPS), multi-atlas + STAPLE. 55 atlases. baseline+1yr of ADNI seg.
%    - atrophy rates
%  Lotjonen2010 - Fast and robust
%    - IBSR/ADNI.  
%  Mouiha2011b - Hippocampal atrophy rates in AD
%    - compare SNT vs FreeSurfer on ADNI
%
...
methods, {\it multi-atlas-based segmentation}, use a series of expertly
segmented MRIs (an atlas library) to act as templates against which a target
image is labelled
\cite{Aljabar2009,Collins2010,Heckemann2011,Leung2010,Lotjonen2010,Leung2010,Wolz2010}.
In the basic method, each atlas is warped to fit a subject's neuroanatomy using
nonlinear registration techniques \citep{Collins1995,Klein2009}.  Atlas labels
are then transformed by this warping and a {\it label fusion} technique, such
as voxel-wise voting, is used to merge the competing label definitions from
each atlas into a final segmentation for a subject.  

Many descriptions of multi-atlas-based hippocampal segmentation algorithms
report relying on an atlas library containing anywhere between 30 and 80
expertly labeled images
\citep{Heckemann2011,Collins2010,Aljabar2009,Leung2010,Lotjonen2010} in order
to achieve performance approach that of a manual rater.  As already noted, the
production of an atlas library requires significant manual effort and
expertise, as does the modification of an existing atlas library in order to
better suit a target population or investigate a specific research hypothesis.
Therefore, in this paper we tackle the problem of producing accurate
segmentations using small numbers of manually segmented atlases. 
...
to basic multi-atlas segmentation: by label propagation and label fusion.  This
intuition driving this approach is to leverage the unique neuroanatomy of
subject population on hand to initialize the segmentation process and improve
accuracy over direct propagation from the atlas library to unlabelled subjects.
...
single atlas and propagating those labels to intermediate subjects before
fusing them in a target image space.  Heckemann compared indirect single-atlas
...
In previous work \citep{Chakravarty2012}, we applied MAGeT brain to
...
extend our approach to the human hippocampus - a much more difficult structure
to segment -- and perform a thorough validation over a range of atlas and
template library sizes, registration methods, and fusion methods.  Our aim in
this work is not necessarily to improve on segmentation accuracy beyond
existing methods, but instead to provide a method that trades off manual
segmentation expertise and time for computational processing time while still
providing sufficient accuracy for clinical and research applications.
