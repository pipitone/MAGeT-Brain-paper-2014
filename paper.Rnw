...
(\ref{fig:ADNI1-xval-variability}) compares the variability in Dice scores of \mb{} vs.
  ...
  agreement between \mb{} and \multiatlas{} segmentation is computed for each
...
disorders, in this experiment we replicate the cross-validation done in
\expadnixval{} a dataset of patients having had a single episode of psychosis. We
also compare \mb{} segmentations with those of two well-known automated
segmentation methods, \fsl{} and \freesurfer{}. 
...
Export whole hippocampal manual segmentation of each subject is produced
...
\freesurfer{}. Firstly, \mb{} is used with the five Winterburn atlas images and
labels as atlases, and a randomly selected subset of 19 psychosis dataset images
as templates. Secondly, \mb{} is run using the same template images, but instead
as atlases we use five additional five first episode psychosis subjects and
corresponding manual segmentations (not included above). Finally, \fsl{} and
\freesurfer{} are run with their default settings to segment the entire dataset.
The \fsl{} \verb+run_first_all+ script was used according to the FIRST user guide
\footnote{http://fsl.fmrib.ox.ac.uk/fsl/fslwiki/FIRST/UserGuide}, and
\freesurfer{} was run using \verb+recon-all -all+. 
...
protocol those used by \freesurfer{} and \fsl{} in the neuroanatomical features
that are delineated (see \citet{Winterburn2013}, and Table
\ref{tab:subfield-segmentation-protocols} in the Discussion below). This
...
number of atlases or templates but quickly plateaus (Figure
\label{fig:FEP-xval}). A maximum similarity score of \Sexpr{snt.mb.kappa.best}
is found when using 9 atlases, 19 templates, ANTS registration, and majority
vote label fusion. 
...
FEP subjects as atlases (Figure \label{fig:FEP-vols}). Both sets of volumes are
...
large range in true values, for instance). Figure \label{fig:FEP-Bland-Altman}
...
hippocampi (the limits of agreement are between $-2482^3mm$ and $-784^3mm$, and
between $-1653^3mm$ and $79^3mm$, respectively), whereas both \mb{} methods show
a much less exaggerated, but conservative bias (limits of agreement between
$-67^3mm$ and $766^3mm$ when using FEP atlases, and between $-333^3mm$ and
$504^3mm$ when using Winterburn atlases). On average, \freesurfer{} and \fsl{}
overestimate hippocampal volume by about $1600 mm^3$ and $800 mm^3$,
respectively. In contrast, on average \mb{} underestimates volumes by about $300
mm^3$ when using FEP atlases, and by about $80mm^3$ when using Winterburn
atlases. 
...
<<fig:FEP-Bland-Altman,dependson='fig:FEP-vols',fig.width=10,fig.height=3,fig.show='hide'>>=
...
\paragraph{Winterburn Atlases}  %%%%%%%%%%
The Winterburn atlases \citep{Winterburn2013} are digital hippocampal
segmentations of five in-vivo $0.3mm$-isotropic T1-weighted MR images, described
more fully in \expfepxval, section \ref{exp:fep-xval}. 

\paragraph{Experiment details} %%%%%%%%%%
\mb{} was configured with an atlas library composed of the five Winterburn and
segmentations. A template library of 19 images were randomly selected from the target
dataset of ADNI subjects, and \ants{} registration and majority vote label fusion
were used as these were found to perform favourably in earlier experiments. 
...
\mb{}) and also passing similary quality control inspection were included in the
analysis. 
...
%%%%%%%%%
<<fig:ADNI-volumes-plot,dependson='ADNI-volumes-prep',fig.show='hide'>>=
...
%%%%%%%%%
<<fig:ADNI-volumes-boxplot,dependson='ADNI-volumes-prep',fig.show='hide'>>=
melted = melt(means.complete,measure.vars=c("FS", "FSL", "MAPER","MAGeT"),
              variable.name="Method", value.name="Volume")
...
%%%%%%%%%
<<fig:ADNI-Bland-Altman,dependson='ADNI-volumes-prep',fig.show='hide',fig.height=4,fig.width=10>>=
...
manual segmetnations for T1 images at $0.9mm^3$-isotropic voxels is inherently
...
@
  \caption{Similarity of \mb{} segmentation of subfields and the resampled
...
ggplot(wa.n.all.vols, aes(x=mean, y=diff)) + 
  ...
  theme(legend.direction = "horizontal", legend.position = "bottom", 
        ...
        axis.text = element_text(size = 8))
@
  \caption{\textbf{Bland-Altman plots comparing unilateral hippocampal subfield volume
...
usage scenarios of researchers and clinicians. \textbf{[TODO: mention that DSC
by it's nature penalizes structures with high surface area-to-vol ratios, so
necessarily penalise all subfield errors.]} \textbf{[TODO: discuss perturbed
segmentation kappa values in relation to MAGeT error]}

Our overlap DSC values demonstrates that we can reliably reproduce segmentations
for the CA1, subiculum, and CA4/dentate subfields. %TODO: fix?
That the CA2/CA3 and
molecular layers are less well reproduced (DSC < 0.5) %TODO: fix!
...
@
  ...
  Variance of segmentation agreement between \mb{} and \multiatlas{} segmentation
