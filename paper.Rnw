...
table \ref{tab:SZFEP-Demographics}. 
...
subset are shown in Table \ref{tab:ADNI1-xvalidation-demographics}. 
...
\begin{figure}
<<ADN1-xval-Bland-Atlman, cache=TRUE, dependson='ADNI1-Xval-prep', fig.width=8, fig.height=4>>=
a_20_tmpls = subset(all_data, 
                      reg_method   == "ANTS" &
                      method.mb    == "Majority Vote" &
                      templates.mb == 20 &
...
subjects = bl
subjects$VISCODE = 'bl' # to match up with adnimerge
...
Comparing total hippocampal volume across segmentation methods, we find close
agreement between manual volumes and MAPER and MAGeT volumes across disease 
categories (Figure \ref{ADNI-scr-volumes-boxplot}). FSL and FreeSurfer both 
produce volumes which are demonstratively larger than manually segmented volumes.
...
pe_gold0.9mm = cbind(subjects,pe_gold0.9mm, res="Resampled")
pe_mbbravo   = cbind(subjects,pe_mbbravo,   res="BRAVO")
pe_mb0.6mm   = cbind(subjects,pe_mb0.9mm,   res="0.9mm voxels")
