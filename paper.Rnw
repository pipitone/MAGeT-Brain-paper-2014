...
% ADNI1-XVAL: mean Kappa plot by atlases, templates, reg and voting method. 
\begin{figure}
<<fig:FEP-xval-k-mean, cache=TRUE, dependson='ADNI1-xval-load', fig.width=7, fig.height=7>>=
fep.xval <-subset(read.csv('data/results-fep-xval-ants-2013-12-28-223825.csv'), k > 0.5)
...
ggplot(fep.xval.mean, aes(x=templates,y=k, colour=atlases)) + 
  ...
  theme(legend.direction = "horizontal", legend.position = "bottom", 
        axis.text = element_text(size = 8))
@
  \caption{Mean Dice's Similarity Coefficient of \mb{} unilateral hippocampal 
  segmentations relative to
  \psegs{} for 81 FEP subjects vs. atlas and template library
  size using \ants registration algorithm and majority vote label fusion method 
  over 3 folds of Monte Carlo cross validation. 
  Error bars indicate standard error.
  \label{fig:FEP-xval-k-mean}}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Schizophrenia dataset. \mb{} is run two ways: with an atlas library consisting
