...
segmentation of standard 3T T1-weighted images at $0.9mm$-isotropic voxels. We
...
\ref{exp:winterburn-xval}) demonstrates the reliability of \mb{} in producing
subfield segmentations which match the segmentation protocol of the input
atlases despite contrast and resolution limitations in standard T1-weighted
image volumes.  All of these experiments together demonstrate that \mb's
algorithmic performance is not dependent on a single definition of the
hippocampus but is effective with differing hippocampal definitions
\citep{Winterburn2013,Pruessner2000,Hsu2002}, across image types, and subject
populations. 
