...
%%%%%                                                                     %%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Experiment 1: Whole Hippocampus Cross-Validation}
...
%%%%%                                                                     %%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Experiment 2: Hippocampal Subfield Cross-Validation}
...
Figure \ref{fig:WAval-kappa} shows the overlap similarity scores between the
...
\begin{figure}
<<fig:WAxval-Bland-Altman>>=
wa.n.vol = read.csv('data/winterburn.xval.volumes.csv')
winterburn = read.csv('data/winterburn.volumes.csv')
...
wa.n.xval.bland.cor = ddply(wa.n.mean.vols,c("variable"),function(df) {
  cor.t = cor.test(df$low,df$mb)
  data.frame(
    pearson = sprintf("r=%0.2f, t=%0.2f, p < %0.2f", 
                cor.t$estimate, cor.t$statistic, cor.t$p.value)
  )
})


ggplot(wa.n.mean.vols, aes(x=mean, y=diff)) + 
  facet_wrap(~variable, ncol=3, scales='free_x') + 
  geom_smooth(method="lm", formula=y~x, fullrange=T, se=F) + 
  geom_point(size=2, aes(colour=hemisphere, shape=subject)) +  
  ...
  geom_text(aes(y = Inf, x=-Inf, label=pearson), colour="black", 
             data = wa.n.xval.bland.cor, hjust=-0.1,vjust=2.5, size=2.5) + 
...
We find that in each subfield, \mb{} shows a proportional bias towards
underestimating volumes in all subregions (Figure
\ref{fig:WAxval-Bland-Altman}).  Prominent fixed biases in the volumes of the
CA4/DG and SR/SL/SM subregions are congruent with the lower similarity scores
...
%%%%%                                                                     %%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Experiment 3: Application to the segmentation of first episode
...
ground truth segmentations of various methods.  Unless otherwise specified,
...
\ref{ab:subfield-segmentation-protocols} for a comparison of manual subfield
