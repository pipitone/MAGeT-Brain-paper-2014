...
methods, \mb is applied to large dataset from the ADNI project and the resulting
segmentations are compared to those produced by FreeSurfer, FSL, MAPER and by
expert semi-automated manual segmentations (SNT).

\paragraph{ADNI1 dataset revisited} 
The {\em \adnidataset} standardized dataset contains $1919$ images in total.
SNT and FreeSurfer hippocampal volumes for a subset of images are provided by
...
atlases (described in Experiment 2), and the size of template library,
