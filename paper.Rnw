...
of images and SNT \gs segmentations from the ADNI1 dataset \citep{Hsu2002}. This
...
Winterburn atlas subjects (referred to as the {\em T1 BRAVO} dataset).  Figure
...
the degree of correlation in average bilateral hippocampal volume of the
subjects produced by each method. Specifically, a Pearson correlation is used. 
...
@
  ...
  size, registration algorithm, and label fusion method. Error bars indictate
  standard error.
...
(Figure \ref{fig:WAval-vol-boxplot}). In particular, the CA1, CA4/Dentate, and
SR/SL/SM subregions all show near or smaller percent errors. The Subiculum and
CA2/CA3 subregions show distinctly larger error than is found through
resampling.  
...
subject images (7 from each disease class) and majority vote label fusion.
...
\begin{figure}
  ...
  \end{centering}
  \caption{SNT \gs and \mb segmentations for 30 ADNI1 subjects (10
