...
\newcommand{\expadnidesc}{Whole Hippocampus Segmentation Comparision --- ADNI1 Complete 1Yr} 
...
existing automated methods (\fsl{}, \freesurfer{}, \maper{}, and \snt) and
biases are explored.\\
...
\begin{itemize}
  \item \expadnixval{} investigates the accuracy of \mb{} whole hippocampus
  ...
  \item \expfepxval{} consists of a similar cross-validation to explore \mb{}
  segmentations on the brain images of young, first episode psychosis patients,
  and to establish parameter settings that do not overfit on a single dataset.  

  \item \expadni{} bridges \mb{} with the existing segmentation literature by
  comparing \mb{} whole hippocampus segmentations with those of well-known
  automated methods (\freesurfer, \fsl, \maper, \snt) on the entire
  \adnidataset{} image dataset consistening of
...
whole hippocampus labels manually segmented following the \citep{Pruessner2000}
protocol. 

Note, in the Supplementary Materials we have replicated this experiment using
the \snt{} semi-automated segmentations included as part of the the ADNI
dataset.
...
This strongly suggests that these results are independant of the segmentation
...
disorders, in this experiment we use the Winterburn atlases
\citep{Winterburn2013} to derive whole hippocampal segmentations of a dataset of
patients having had a single episode of psychosis. The resulting segmentations
are assessed for quality by comparison with expert manual segmentations.
...
the unlabelled images also serve as template images. Majority vote fusion, and
...
using \mb{} and two other popular segmentation packages, \fsl{} and
\freesurfer{}. Firstly, \mb{} is used with the five Winterburn atlas images and labels as atlases, and a
randomly selected subset of 19 psychosis dataset images as templates. Secondly,
\mb{} is run using the same template images, but instead as atlases we use five
additional five first episode psychosis subjects and corresponding manual
segmentations (not included above). Finally, \fsl{} and \freesurfer{} are run
with their default settings to segment the entire dataset. The \fsl
\verb+run_first_all+ script was used according to the FIRST user guide
\footnote{http://fsl.fmrib.ox.ac.uk/fsl/fslwiki/FIRST/UserGuide}, and
\freesurfer was run using \verb+recon-all+. \textbf{
...
The Pruessner segmentation protocol differs slightly from the Winterburn
protocol those used by \freesurfer and \fsl in the neuroanatomical features that
are delineated (see \citet{Winterburn2013}, and Table
\label{tab:subfield-segmentation-protocols} in the Discussion below).  This
variation in protocol poses a problem if an overlap measure is used for
evaluation: since different protocols will necessarily produce segmentations
that do not perfectly overlap, the degree of overlap cannot be solely used to
compare segmentation methods using different protocols. In place of an overlap
metric, we assess the degree of (Pearson) correlation in average bilateral
hippocampal volume produced by each method. Additionally, we evaluate the
...
atlases (described in \expsubfield{}, section \ref{exp:winterburn-xval}), and size of template library,
registration method, and label fusion method were determined by the optimal
settings found in \expadnixval{} (Section \ref{exp:adni-xval}).  The \fsl{} segmentation method was used via
...
Based on the results from \expadnixval{} (Section \ref{exp:adni-xval}), in this
...
%^ TODO: winterburn atlases described in expfepxval
\paragraph{Experiment details} 
...
method found in \expadnixval{} are used. 
...
of \mb{} segmentations using DSC overlap scores and evaluate consistency
...
to provide accurate and reliable segmentations.  \expadnixval{} (Section
...
and allows us to choose optimal settings for subsequent experiments.
\expfepxval{} (Section \ref{exp:winterburn-xval}) demonstrates the reliability of
...
T1-weighted image volumes.  \expadnixval{} and \expfepxval{} validate whole hippocampal
...
\textbf{[TOOD: are all experiments referred to here?]}
...
template library from a small atlas set \mb{} is able to improve the final
segmentation similarity. The magnitude of this improvement is greatest with a
