...
mean Dice's Similarity Coefficient (DSC) of \Sexpr{round(mb.best.kappa, digits=3)} 
...
estimated from non-linear image registration. The resulting candidate labels
...
(Figure \ref{fig:ADNI1-xval-variability}).  The size of template library 
