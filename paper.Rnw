...
\ref{exp:fep-xval}) are resampled to $0.9mm$-isotropic
...
\ref{tab:WAxval-perturb-kappa}. We find that the CA4/DG subfield shows the highest mean DSC score of
...
of\Sexpr{mean(Whole)}$\pm$\Sexpr{sd(Whole)}. Table
\ref{tab:WAxval-perturb-kappa} also shows mean DSC overlap scores for simulated
manual segmentations with error introduced by a one-voxel translation in all
directions. For all subfields, \mb{} DSC scores are well within the error bounds
set by these segmentations.
