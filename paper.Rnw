...
\textbf{Results:} 
ADNI-XVAL
In the ADNI cross-validation, using 9 atlases \mb{} achieves a mean Dice's
Similarity Coefficient (DSC) score of \Sexpr{mb.kappa.best} with respect to
manual whole hippocampus segmentations, and also exhibits significantly lower
variability in DSC scores than \multiatlas{} segmentation.  In the younger,
psychosis dataset, \mb{} achieves a mean DSC score of \Sexpr{fep.mb.kappa.best}
and produces volumes which agree with manual segmentation volumes better than
those produced by the \freesurfer and \fsl methods (mean difference in volume:
$80mm^3$, $1600mm^3$, and $800mm^3$, respectively). Across the \adnidataset
dataset,
...
of the AD hippocampus compared to the \snt{} segmentations. 
