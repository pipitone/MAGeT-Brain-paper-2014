...
using our \multiatlas framework.  To ensure that we have not over-fit our
...
\begin{figure}[h!]
<<WAval-vol-plots,cache=T,fig.width=6,fig.height=6>>=
...
ggplot(wa.xval.volumes, aes(x=gold_0.3mm, y=volume, colour=measure)) + 
  ...
  geom_text(aes(x=Inf-200, y=Inf-200, label=eqn), parse=TRUE, data=wa.xval.vols.lm, position="dodge") +
  theme(legend.direction = "horizontal", legend.position = "bottom")
