...
A total of 10 validation rounds are perfomed on each subject in the dataset, for each 
combination of parameter settings: atlas library size (1-9), template library 
size (1-20), registration method (ANTS vs. ANIMAL), and label fusion method 
(MV, XC-WV, NMI-WV). A total of $10*69*9*20*2*3 = 7452000$ validation rounds were
conducted, resulting in a total of 1'490'400 segmentations analysed. 
...
between labels produced by \mb and the ADNI labels.  To evaluate the
performance of \mb, we compare the correlation of \mb segmentation
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% ADNI1 Screening Template Library Demographics Table
%
...
t_data       = read.csv(gzfile('data/a2a_tracc/results_2012_12_31.csv.gz'))
...
each of 69 subjects, over a range of atlas and template library sizes, with ANTS 
and ANIMAL registration algorithms, and three kinds of label fusion techniques. 
Hippocampal \mb-based segmentations using both ANIMAL and ANTs registration
...
atlas library and template library. With 12 or more templates \mb library of at 
least 12 templates 
...
ANTs registration shows increased segmentation accuracy over \mb with ANIMAL registration (\todo{interaction?}).
...
(\todo{show significance of this}). See section (\todo{ref}) for a discussion of
this behaviour. In the remainder of this section, only results from odd-sized
template libraries will be shown. 
...
ggplot(all_data_mean, 
  ...
  ylab( "Mean DSC" )
@
  \caption{\mb mean DSC by atlas and template library size}
...
@
  \caption{Increase in mean Kappa between \mb and multi-atlas}
...
ANTSmajvote = subset(all_data, reg_method=="ANTS" & method.mb=="Majority Vote")
...
ggplot(stats.tests, 
  ...
  scale_x_continuous(breaks=seq(1,20,by=2)) + scale_y_continuous(breaks=seq(0,1,by=0.05)) + 
  xlab( "Number of Templates" ) + 
  ylab( "p-value" ) 
...
% @
%   \caption{Change in mean Kappa between multi-atlas and \mb using ANTS, 20 templates}
%   \label{}
% \end{figure}
% 
% 
...
qc_pruned     = qc[,c("Image.Data.ID","MAGeT_QC","FSL_QC")]
qc_pruned     = subset(qc_pruned, MAGeT_QC == 1 & FSL_QC == 1)
...
combined = merge(combined, qc_pruned   , by=c("Image.Data.ID"))
...
@
  \caption{Comparison of HC volumes by FreeSurfer (FSF), \mb (MAGeT), MAPER, and manual (SNT).}
