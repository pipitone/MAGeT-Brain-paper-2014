...
levels(melted$measure) <- list("Subsampled atlas labels"="resampled",
                               "MAGeT on subsampled atlas images" = "mb_0.9mm",
                               "MAGeT on T1 BRAVO images"="mb_bravo")

error.t.tests = ddply(error, c("region"), function (df) { 
  data.frame( 
    bravo = round(t.test(df$resampled, df$mb_bravo, paired=T)$p.value,3), 
    atlas = round(t.test(df$resampled, df$mb_0.9mm, paired=T)$p.value,3)
  )}) 
@
...
ggplot(melted, aes(y=percenterr,x=measure,colour=measure)) +
  facet_grid(.~region) + 
...
%                        
<<ADNI-xvals-pvalue, echo=F, results="asis",cache=FALSE>>=
latex(error.t.tests, file="", size="scriptsize",landscape=FALSE, ctable=TRUE,
    caption="\\textbf{t-test p-values whem comparing to resampled percent error.}", 
    caption.loc = c("top"), rowname=NULL, 
    label="tab:resampled-percent-error")
@
