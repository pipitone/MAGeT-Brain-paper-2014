...
caption = paste(
  ...
  "Clinical Dementia Rating-Sum of Boxes (CDR-SB), Alzheimer's Disease",
  "Assessment Scale (ADAS), Mini–Mental State Examination (MMSE).", sep=" ") 
...
\begin{table*}[!tbp]
\scriptsize
\caption{\textbf{Subfield validation template library demographics.}
\label{tab:WAval-healthy-demographics}}
\begin{center}
\begin{tabular}{l | c c c}
\hline 
Parameters        & Stage 1 & Stage 2 & Stage 3  \tabularnewline
...
with the segmentations themselves from ADNI \cite{Hartig2010}. One author (MP) 
inspected 

Only those images which had segmentations from every method
...
latex(package_totals, file="", size="scriptsize",landscape=FALSE, ctable=TRUE,
    caption="\\adnidataset dataset segmentation totals and QC failures.", 
...
have only ever performed cross-validation \citep{?}.  This suggests that
