...
achieving comparable segmentation accuracy while using significantly fewer
manually created atlases.
...
fusion method (for example, by voxel-wise majority voting). 
