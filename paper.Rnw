...
%                                                                              %
%                                                                              %
 % Bland-Altman plot for MA and MAGeT 
...
ggplot(subset(all_data, reg_method=="ANTS" & method.y=="Majority Vote"), 
  ...
  scale_x_continuous(breaks=1:20) + scale_y_continuous(breaks=seq(-1,1,by=0.01)) + 
  scale_colour_hue(name="Atlases") +
...
stddev = ddply(ANTSmajvote, c("subject", "label","atlases","templates.y"), function (df) {
  data.frame(
    MA = sd(df$k.x),
    MB = sd(df$k.y) 
  )
...
stddev.t.test = ddply(stddev, c("atlases","templates.y"), function (df) {
  t = t.test(df$MA, df$MB)
  data.frame(
...
@
  \caption{{\bf ADNI Baseline cohort.} Comparison of total hippocampal volumes as measured by SNT, FSL, FreeSurfer (FS), MAPER, and MAGeT brain (MAGeT). A fitted normal curve is shown in red. $W$ is the Shapiro-Wilkes test statistic measuring normality of the data (signficance indicates non-normality). $D$ is the Kolmogorov-Smirnov test statistic measuring the goodness-of-fit between the distribution of measured volumes and SNT volumes (significance indicates a difference).}
...
melted = melt(totals.complete, measure.vars=c("SNT","FSL", "FS", "MAPER"))
